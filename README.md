...
> **一句话总结**：本项目是面向国内初学者的开源大模型全流程实践教程，涵盖环境配置、本地部署、高效微调，支持 50+ 主流大模型，适配 Linux / AMD / 华为昇腾 / Apple M 系列等多平台。

| 📌 关键信息 | 详情 |
|---|---|
| **项目定位** | 开源 LLM 部署、使用与微调的中文实战教程库 |
| **目标受众** | 国内学生、研究者、开发者（无需深厚技术背景）|
| **覆盖模型** | 50+ 主流大模型（Qwen、GLM、DeepSeek、LLaMA、InternLM 等）|
| **教程内容** | 环境配置 → 本地部署 → API 服务 → WebDemo → LoRA 微调 → RAG / Agent |
| **支持平台** | Linux x86（主线）、AMD GPU、华为昇腾 NPU、Apple M 系列（MLX）|
| **示例项目** | Chat-嬛嬛、天机、AMChat、数字生命 等完整应用案例 |
| **维护方** | [Datawhale](https://github.com/datawhalechina) 开源社区，80+ 贡献者 |
